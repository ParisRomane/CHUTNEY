...
def update_lobby(sended,socket):
	global data, current_port
	if sended.startswith('LOBBY HOST'):
		sended = sended.split()
		json = {'max_joueur' : 4, 'nb_joueur' : 1, 'joueur1': {'pseudo': sended[2]}, "port": current_port }
		data.append(json)
		system("python3 multiplayer/server_game.py "+ str(current_port)+" &")
		current_port += 1
		print(data)
